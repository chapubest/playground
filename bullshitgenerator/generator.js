...
// 论述
let discuss = [ 
    "现在，解决theme的问题，是非常非常重要的。 所以， ",
    "我们不得不面对一个非常尴尬的事实，那就是， ",
    "theme的发生，到底需要如何做到，不theme的发生，又会如何产生。 ",
    "而这些并不是完全重要，更加重要的问题是， ",
    "theme，到底应该如何实现。 ",
    "带着这些问题，我们来审视一下theme。 ",
    "所谓theme，关键是theme需要如何写。 ",
    "我们一般认为，抓住了问题的关键，其他一切则会迎刃而解。 ",
    "问题的关键究竟为何? ",
    "theme因何而发生?",
    "每个人都不得不面对这些问题。 在面对这种问题时， ",
    "一般来讲，我们都必须务必慎重的考虑考虑。 ",
    "要想清楚，theme，到底是一种怎么样的存在。 ",
    "了解清楚theme到底是一种怎么样的存在，是解决一切问题的关键。 ",
    "就我个人来说，theme对我的意义，不能不说非常重大。 ",
    "本人也是经过了深思熟虑，在每个日日夜夜思考这个问题。 ",
    "theme，发生了会如何，不发生又会如何。 ",
    "在这种困难的抉择下，本人思来想去，寝食难安。 ",
    "生活中，若theme出现了，我们就不得不考虑它出现了的事实。 ",
...
function getRandom(min = 0, max = 100){
    let seed = Math.random()*( max - min ) + min;
    return seed;
}
// 获取名人名言
function getQuote(){
    let word = getListRandom(quote);
    word = word.replace("曾经说过", getListRandom(before) )
    word = word.replace("这不禁令我深思", getListRandom(after) )
    return word
}

function getDiscuss(){
    let word = getListRandom(discuss);
    word = word.replace(RegExp("theme", "g"),theme);
    return word;
}
